    ...
    >
      ...
      >
        <template v-slot:item.id="{ item }">
          <span class="font-weight-bold text-indigo-darken-4 text-uppercase"
            >#{{ item.id }}</span
          >
        </template>
        ...
        <template v-slot:item.order_id="{ item }">
          <v-btn
            ...
            :to="{ name: 'admin-order', query: { open_order: item.order_id } }"
          >
            #{{ item.order_id }}
          </v-btn>
        </template>
        ...
        <template v-slot:item.transaction_type="{ item }">
          <v-chip
            size="small"
            :color="
              item.transaction_type === 'payment' ? 'success' : 'red-darken-2'
            "
            variant="flat"
        ...
        <template v-slot:item.payment_method="{ item }">
          <div class="d-flex align-center">
            <v-icon size="18" class="mr-1" color="grey-darken-2">
              {{
                item.payment_method === "Cash" || item.payment_method === "COD"
                  ? "mdi-cash"
