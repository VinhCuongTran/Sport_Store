      ...
      >
        ...
        <v-card-actions class="px-6 py-4 gap-3 bg-white">
          ...
          <v-btn
            ...
            variant="tonal"
            rounded="lg"
            class="text-capitalize font-weight-bold mr-auto"
            @click="printExportTicket"
          >
            <v-icon start>mdi-printer</v-icon>In Phiếu Xuất
          </v-btn>
          <!-- KẾT THÚC THÊM -->
          <v-spacer />
          <v-btn
...
const printExportTicket = () => {
  ...
  order.items.forEach((item, index) => {
    ...
    rows += `
      <tr>
        ...
        <td>${item.product_name} (${item.color} - ${item.size})</td>
  ...
  printWindow.document.write(`
    <html>
      <head>
        <title>Phiếu Xuất Kho - ĐH #${order.id}</title>
        <style>
      ...
      <body>
        <div class="header">
          ...
          <div class="ticket-meta text-right">
            <p>Mã đơn hàng: <strong>${order.id.toUpperCase()}</strong></p>
          </div>
        </div>
        
        <div class="ticket-title">
          <h1>PHIẾU XUẤT KHO KIÊM GIAO HÀNG</h1>
        ...
        <div class="info-section">
          ...
          <p><strong>Ghi chú:</strong> Xuất bán hàng hóa</p>
        </div>
        ...
        <table>
          ...
          <tfoot>
            <tr>
              ...
              <th></th>
              <th class="text-right">${formatPrice(order.total_price)}</th>
            </tr>
          </tfoot>
        </table>
