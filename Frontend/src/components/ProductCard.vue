<template>
  <v-card
    class="product-card border rounded-xl overflow-hidden bg-white"
    elevation="0"
  >
    ...
    <v-card-title
      class="text-h6 font-weight-bold text-black pt-2 pb-0 text-truncate"
    >
      {{ product.name }}
    </v-card-title>

    <v-card-subtitle class="text-grey-darken-1 pb-2 mt-1">
      Thương hiệu: {{ product.brand_name || "Khác" }}
    </v-card-subtitle>

    <v-card-text class="py-1">
      <div class="font-weight-bold text-red-darken-2 text-h6">
    ...
    <v-card-actions class="px-4 py-3 bg-grey-lighten-4 border-t">
      <router-link
